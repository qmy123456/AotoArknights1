...
def isSwapWorker(workers, checkList):
    ...
    for i in workers[0]:
        if i in l:
            return 'a'
        if workers[1] < config.chijinMood:
            return 'b'
    return 'c'


# 换班逻辑
def chijinhuanban(p):  # swapord 0需要选中的干员 1以选中干员
    swapord = [[], []]
    # 贸易站换干员逻辑
    ...
    swapres = isSwapWorker(getWorkersMood(ocrres), config.bChijin)
    ...
    if swapres == 'a':
        ...
        operation.screenshot()  # 截屏
        ocrres = operation.myPaddle.ocr(config.exeImg)  # 识别结果
        jijianTool.clickPageWorkers(ocrres, config.aChijin, swapord)
        if len(swapord[1]) == 3:
        ...
        operation.screenshot()  # 截屏
        ocrres = operation.myPaddle.ocr(config.exeImg)  # 识别结果
        jijianTool.clickPageWorkers(ocrres, config.bChijin, swapord)
        if len(swapord[1]) == 3:
            operation.clickNextNode((1170, 680))
            print('更换成功')
        elif len(swapord[1]) < 3:
...
def start():
    chijinhuanban(config.chijinP1)
    chijinhuanban(config.chijinP2)
